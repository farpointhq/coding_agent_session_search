...
fn doctor_json_reports_missing_upstream_source_as_coverage_risk_not_data_loss() {
    ...
    // The raw-mirror capture/backfill write path has been removed, so doctor no
    // longer emits a raw_mirror_backfill plan/receipt. The data-loss protection
    // that mattered here now reads straight from the archive DB: the missing
    // upstream source is reported as a sole-copy coverage risk (asserted via
    // coverage_summary / coverage_risk / sole_copy_warnings below), and exact
    // source paths still must not leak.
    assert!(
